...
class BaseDataSets(Dataset):
    def __init__(
        ...
        self.un_label_num = un_label_num
        ...
        if self.split == "train":
            with open(self._base_dir, "r") as f1:
                 a = f1.readlines()
            self.sample_list.extend(a)
            new_filename = self._base_dir.replace("train1", "train2")
            with open(new_filename, "r") as f1:
                a = f1.readlines()
            self.sample_list.extend(a)
            new_filename = self._base_dir.replace("train1", "train3")
            with open(new_filename, "r") as f1:
                a = f1.readlines()
            self.sample_list.extend(a)
            ...
            if sign==1:
                new_filename = self._base_dir.replace("train1", "untrain1")
                with open(new_filename, "r") as f2:
                    self.unlabel = f2.readlines()[:self.un_label_num]
                self.sample_list.extend(self.unlabel)
                new_filename = self._base_dir.replace("train1", "untrain2")
                with open(new_filename, "r") as f2:
                    self.unlabel = f2.readlines()[:self.un_label_num]
                self.sample_list.extend(self.unlabel)
                new_filename = self._base_dir.replace("train1", "untrain3")
                with open(new_filename, "r") as f2:
                    self.unlabel = f2.readlines()[:self.un_label_num]
                self.sample_list.extend(self.unlabel)
    ...
    def __getitem__(self, idx):
        ...
        label1 = h5f["label1"][:]
        label2 = h5f["label2"][:]
        if (idx>=70 and idx<150) or (idx>=315 and idx<395):
        ...
        elif (idx>=150 and idx<245) or (idx>=395 and idx<490):
...
def get_dataset(name,sign,patch_size):
    if name == 'huaxiFG':
        ...
        train_dataset_1 = BaseDataSets(base_dir=dir_img1, split="train", num=None, sign=sign, un_label_num=un_label_num,
                                       transform=transforms.Compose([RandomGenerator(patch_size)]))

        train_dataset_2 = BaseDataSets(base_dir=dir_img2, split="train", num=None, sign=sign, un_label_num=un_label_num,
                                       transform=transforms.Compose([RandomGenerator(patch_size)]))

        train_dataset_3 = BaseDataSets(base_dir=dir_img3, split="train", num=None, sign=sign, un_label_num=un_label_num,
                                       transform=transforms.Compose([RandomGenerator(patch_size)]))
...
if __name__ == '__main__':
    if __name__ == '__main__':
        a, b = get_dataset('huaxiFGOnly', 1, 140, [256, 256])
        # print(len(a[0][0]))
        # print(len(b))
